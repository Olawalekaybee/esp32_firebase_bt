 ...
 * ABOUT:
 *
 * The non-blocking (async) example to sign up anonymously as guest.
 *
...
void setup()
{
    ...
    signup(aClient, app, getAuth(user), asyncCB, "signupTask");
}
...
void loop()
{
    ...
    if (app.ready() && (millis() - ms > 30 * 1000 || ms == 0))
    {
        ...
        if (cnt == 3)
        {
            ...
            UserAccount user(API_KEY);
            deleteUser(aClient, app, getAuth(user.idToken(app.getToken())), asyncCB, "deleteUserTask");
        }
    }
}
