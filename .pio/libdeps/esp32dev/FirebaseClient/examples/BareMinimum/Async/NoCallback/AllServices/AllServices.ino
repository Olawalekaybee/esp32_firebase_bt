 ...
 * ABOUT:
 *
 * The beare minimum code example for all Firebase services in async mode without callback function.
 *
...
void loop()
{
    ...
    if (app.ready() && !onetimeTest)
    {
        ...
        messaging.send(aClient, Messages::Parent("PROJECT_ID"), msg, myResult);

        // Firestore get document.
        Docs.get(aClient, Firestore::Parent("PROJECT_ID"), "info/countries", GetDocumentOptions(DocumentMask("Singapore")), myResult);

        // Firebase Storage get object metadata
        storage.getMetadata(aClient, FirebaseStorage::Parent("STORAGE_BUCKET_ID", "media.mp4"), myResult);

        // Cloud Storage list objects.
        GoogleCloudStorage::ListOptions options;
        options.maxResults(3);
        options.prefix("media");
        cstorage.list(aClient, GoogleCloudStorage::Parent("STORAGE_BUCKET_ID"), options, myResult);

        // Cloud Functions call function.
        cfunctions.call(aClient, GoogleCloudFunctions::Parent("PROJECT_ID", "PROJECT_LOCATION"), "helloWorld", "test", myResult);
    }
